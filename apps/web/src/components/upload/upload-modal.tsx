  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 w-11/12 max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white p-6 shadow-xl dark:bg-zinc-900">
          <Dialog.Title className="text-lg font-semibold">Add a new document</Dialog.Title>
          ...
          >
            <input {...getInputProps()} />
            {isDragActive ? <p>Drop the file(s) here…</p> : <p>Drag & drop PDF or images here,<br />or click to browse</p>}
          </div>

          {progress && (
            <div className="mt-4">
              <p className="text-sm text-zinc-600 dark:text-zinc-300">{progress.stage.replace('-', ' ')}</p>
              <div className="mt-2 h-2 w-full overflow-hidden rounded bg-zinc-200 dark:bg-zinc-800">
                <div className="h-full bg-blue-600 transition-[width]" style={{ width: `${progress.pct}%` }} />
              </div>
            </div>
          )}
